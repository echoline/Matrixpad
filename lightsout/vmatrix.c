...
Matrix *newmat(unsigned int r, unsigned int c){
	...
	m->d = malloc(sizeof(double*) * m->r);
	for(i=0; i < m->r; i++){
		m->d[i] = calloc(1, sizeof(double) * m->c);
	}
...
void ident(Matrix *m){
	unsigned register r, c;

	if (m == nil || m->d == nil || m->r != m->c)
		return;

	for (r=0; r<m->r; r++) for (c=0; c<m->c; c++){
		if (r == c){
			m->d[r][c]=1;
		}
		else{
